# Tests of both msigdbrdata() output format and specific database contents
...
test_that("msigdbrdata() human", {
  ...
  expect_identical(unique(mdb$db_version), "2026.1.Hs")
  ...
  expect_false(anyNA(mdb$db_gene_symbol))
  expect_false(anyNA(mdb$db_ncbi_gene))
  expect_false(anyNA(mdb$db_ensembl_gene))
  expect_false(anyNA(mdb$source_gene))
  expect_false(anyNA(mdb$gs_id))
  expect_false(anyNA(mdb$db_version))
  expect_false(anyNA(mdb$db_target_species))
  # overall dimensions
  expect_equal(n_distinct(mdb$gs_collection), 10)
  ...
  expect_equal(nrow(filter(mdb_sym, gs_id == "M30053")), 1963)
  expect_equal(nrow(filter(mdb_ens, gs_id == "M30053")), 1964)
...
test_that("msigdbrdata() mouse", {
  ...
  expect_equal(n_distinct(mdb$db_version), 1)
  expect_identical(unique(mdb$db_version), "2026.1.Mm")
